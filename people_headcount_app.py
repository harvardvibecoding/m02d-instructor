...
### m02d-instructor/people_headcount_app.py
###
...
if not selected.empty:
    chart_df = selected.reset_index().loc[:, ["name", "comp_usd"]].rename(columns={"name": "Name", "comp_usd": "Compensation"})
    chart = (
        alt.Chart(chart_df)
        .mark_bar()
        .encode(
            x=alt.X("Compensation:Q", title="Compensation (USD)"),
            y=alt.Y("Name:N", sort=alt.EncodingSortField(field="Compensation", op="sum", order="descending"), title=None),
            color=alt.value(HARVARD_CRIMSON),
            tooltip=["Name", alt.Tooltip("Compensation:Q", format="$,.0f")],
        )
